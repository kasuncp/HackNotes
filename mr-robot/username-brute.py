...
def check_response(content, user):
    if "invalid username or e-mail" in content.lower():
        print(bcolors.FAIL + "+ -- --=[Wrong username: " + user + "" + bcolors.ENDC)
    else:
        print(bcolors.OKBLUE +
            "+ -- --=[User found: " + user + "" + bcolors.ENDC)
        sys.exit(0)
...
def template2(user):
    t = "user_login=%s&redirect_to=&wp-submit=Get+New+Password" % user
    return t
